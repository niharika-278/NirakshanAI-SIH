...
Combines already-cleaned, state-wise MPLADS CSV files into two national-level
datasets for the ML team:
...
PROJECTS_SUFFIX = "_sanctioned_projects.csv"
        ...
        for fname in sorted(files):
            ...
            if fname.endswith(PROJECTS_SUFFIX):
                ...
                if slug in projects_files:
                    warnings.append(
                        f"Multiple '*_sanctioned_projects.csv' files found for state '{slug}'. "
...
def main():
    ...
    for slug in all_state_slugs:
        if slug not in projects_files:
            missing_notes.append(f"{display_state_name(slug)}: no '*_sanctioned_projects.csv' file found")
    ...
    for slug in all_state_slugs:
        ...
        df, err = read_csv_safely(path)
        if err:
            schema_issues.append(f"{display_state_name(slug)} sanctioned_projects ({path}): {err} -> EXCLUDED")
            log(f"{display_state_name(slug):<14}{'ERROR':>10}  ({err})")
            continue
        if df is None or len(df.columns) == 0:
            schema_issues.append(f"{display_state_name(slug)} sanctioned_projects ({path}): no columns -> EXCLUDED")
            log(f"{display_state_name(slug):<14}{'ERROR':>10}  (no columns)")
            continue

        label = f"{display_state_name(slug)} sanctioned_projects ({os.path.basename(path)})"
